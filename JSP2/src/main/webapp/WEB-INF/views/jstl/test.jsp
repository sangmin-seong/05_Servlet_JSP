...
<hr>
<h3>3. 단일 조건문인 c:if</h3>

<pre>
  - Java에 if문을 태그 형태로 만든 것
...
[c: choose 태그]
  ...
  * 주의 사항 *
    바로 한 단계 아래에는 c:when, c:otherwise, 공백, jsp 주석만 작성할 수 있고,
    나머진 모두 에러 발생!!
...
<c:choose>
   ...
   <%--
    ...
    - EL에서 사용하는 연산자는
      ...
      true를 반환하고, 나머지 경우에는 false를 반환한다
    --%>
   <c:when test="${empty param.age}">
    <h4 style = "color : red">작성된 내용이 없습니다.</h4>
   </c:when>
...
<pre>
  ...
  <ul>
    <li>nameList : ${nameList}</li>
      ...
      --%>
    <li>nameList의 길이(저장된 데이터 개수) : ${fn:length(nameList)}</li>
  </ul>
